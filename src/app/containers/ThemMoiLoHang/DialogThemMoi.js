...
DialogThemMoi.propTypes = {};

function DialogThemMoi({ visible, onCancel, onSubmit, data, disabled, dialogData, viewDialog, onRemoveImage }) {
  ...
  const handleSubmit = (e) => {
    ...
    if (dialogData) {
      let newData = data;
      const index = newData.indexOf(dialogData);
      idProcedureOld = newData[index]._id;
    ...
    if (idProcedureOld) dataSubmit._id = idProcedureOld;

    let foundItem = false;
    if (data != null) {
      foundItem = data.find((item) => item.stepIndex === dataSubmit.stepIndex);
    }
    if (foundItem) toast(CONSTANTS.ERROR, TOAST_MESSAGE.PARCEL.DUPLICATE_STEP);
    else {
      const dataNew = [...data, dataSubmit];
      dataNew.sort((a, b) => a.stepIndex - b.stepIndex);
      onSubmit(dataNew);
      onClose();
    }
  };
