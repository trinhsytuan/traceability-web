import { CloseOutlined, DownloadOutlined, EditOutlined, LeftOutlined, SearchOutlined } from '@ant-design/icons';
...
import {
  createParcel,
  deleteParcel,
  editStepChangeProcedure,
  updateParcelAll,
  updateStep,
} from '@app/services/ThemMoiQuyTrinh';
...
import QuyTrinhXetDuyet from '@containers/QuyTrinhXetDuyet/QuyTrinhXetDuyet';
import { URL } from '@url';
...
import DialogThemMoi from './DialogThemMoi';
...
ThemMoiLoHang.propTypes = {};

function ThemMoiLoHang({ myInfo, myPermission }) {
  ...
  const [showGuiDuyet, setShowGuiDuyet] = useState(false);
  ...
  const GenerateQRCode = (infoParcel) => {
  ...
  const getAPI = async (
    ...
    if (!idParcel) {
      ...
      //Phần xử lý button gửi duyệt ẩn đi nếu ko có gì
      let statusBtGuiDuyet = false;
      for (let i = 0; i < responseStep.length; i++) {
        if (
          !responseStep[i]?.status_producer ||
          responseStep[i]?.status_producer ===
          STATUS_PARCEL_ENDORSER.CREATING ||
          responseStep[i]?.status_producer ===
          STATUS_PARCEL_ENDORSER.BROWSING ||
          responseStep[i]?.status_producer === RESULT_SENDING.DENIED
        ) {
          statusBtGuiDuyet = true;
          break;
        }
      }
      setShowGuiDuyet(statusBtGuiDuyet);
    }
  };
  const handleBack = () => {
    history.goBack();
  ...
  const handleViewParcel = (data, status) => {
    setShowDialog(true);
    setDialogData(data);
    if (!enableForm) handleViewDialog(status);
  };
  const handleForm = async (e) => {
    ...
    if (!idParcel) {
      const response = await createParcel(dataLoHang, procedureStep);
      if (response) {
        toast(CONSTANTS.SUCCESS, TOAST_MESSAGE.PARCEL.ADD_SUCCESS);
        history.push(URL.CHI_TIET_LO_SAN_PHAM_ID.format(response._id));
      }
      setLoading(false);
    } else if (idParcel && e.procedure == statusEndorser?.procedure?._id) {
      setLoading(true);
      const response = await updateParcelAll(
        idParcel,
        procedureStep,
        dataLoHang,
      );
      await updateStep(procedureStep, removeImg);
      await handleGetID();
      if (response) {
      ...
      const response = await editStepChangeProcedure(
        procedureStep,
        idParcel,
        idProduct,
        statusEndorser.procedure._id,
        dataLoHang,
      );
      if (response) {
        await handleGetID();
  ...
  const handleEditStep = (values) => {
    if (values.status_endorser == RESULT_SENDING.REJECT) return true;
    switch (values.status_producer) {
      case STATUS_PARCEL_ENDORSER.BROWSING:
        return true;
      case STATUS_PARCEL_ENDORSER.CREATING:
        return true;
      case STATUS_PARCEL_ENDORSER.DENIED:
        return true;
      case STATUS_PARCEL_ENDORSER.REJECT:
        return true;
      case null:
        return true;
      case undefined:
  ...
  return (
    <>
            ...
            <>
              {(form.getFieldValue('nguoipt') == myInfo._id ||
                ...
                showGuiDuyet && (
                  <Tooltip
                    placement="top"
                    title="Gửi duyệt thông tin lô hàng tới người quản trị"
                    color="#1890FF"
                  >
                    <Button
                      type="primary"
      ...
      <Loading active={isLoading}>
          ...
          {procedureStep && (
            <div className="show-procedure--lohang">
              <div className="show-procedure--title">
                ...
                {/* {!enableForm && (
                <Button icon={<PlusOutlined />} className="button_reverse" type="primary" onClick={showDialogThemMoi}>
                  Thêm quy trình
                </Button>
              )} */}
              </div>
                        ...
                        <div className="show-procedure--content--title">
                          ...
                          <div>
                            <Button
                              ...
                              style={{ border: 0 }}
                            />
                            {!enableForm && handleEditStep(data) && (
                              <Button
                                icon={<Editable/>}
                                style={{ border: 0 }}
                                onClick={() => {
        ...
        {!enableForm && (
          <div className="add-procedure-btn-add-parcel">
            <Button
              type="primary"
              htmlType="submit"
              form="form"
...
export default connect(mapStatetoProps)(ThemMoiLoHang);
