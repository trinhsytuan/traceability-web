...
import { createTuVan } from "@app/services/TuVan";
...
function ModalTuVan({ onOpen, setIsOpen }) {
  ...
  const finishForm = async(e) => {
    form.resetFields();
    const response = await createTuVan(e);
    if (response) {
    ...
    <Modal title="Đăng ký tư vấn" visible={onOpen} onCancel={setIsOpen} footer={null}>
      <Form size="normal" labelCol={{ span: 7 }} wrapperCol={{ span: 17 }} onFinish={finishForm} form={form}>
        ...
        <Form.Item
          label="SĐT của bạn"
          name="phone"
          rules={[{ required: true, message: "Vui lòng điền tên của bạn!" }, RULES.PHONE_NUMBER]}
        >
          <Input />
        </Form.Item>
        <div
          className="btn-actions"
          style={{ display: "flex", flexDirection: "row", alignItems: "center", justifyContent: "center", gap: "18px" }}
        >
          <Button type="white" onClick={setIsOpen}>
            Huỷ
          </Button>
